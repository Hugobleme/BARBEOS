      ...
      <PublicLayout>
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center max-w-md mx-auto">
          ...
          <h2 className="text-xl font-bold font-serif mb-2">
            Uma nova versão do BARBEOS está disponível.
          </h2>
          <p className="text-muted-foreground mb-8">
            Atualize a página para acessar a melhor experiência.
          </p>
          <Button
  ...
  return (
    <PublicLayout>
      <div className="flex flex-col items-center justify-center py-20 px-4 text-center max-w-md mx-auto">
        ...
        <h2 className="text-xl font-bold font-serif mb-2">
          Não conseguimos acessar a agenda neste momento.
        </h2>
        <p className="text-muted-foreground mb-8">
          Aconteceu um erro inesperado. Tente novamente ou retorne para a lista.
        </p>
        <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto justify-center">
          ...
          <Button asChild className="font-bold uppercase tracking-wider text-xs">
            <Link to="/barbearias">Encontrar outro local</Link>
          </Button>
        </div>
      </div>
    </PublicLayout>
...
function BookingPage() {
  ...
  useEffect(() => {
    if (proId === "any" && pros.length > 0 && step > 1) {
      /* maybe reset? but any is fine */
    }
  }, [proId, pros, step]);
  useEffect(() => {
    ...
    return (
      <PublicLayout>
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
          ...
          <p className="text-muted-foreground mb-8">
            Escolha a unidade desejada para agendar o seu horário exclusivo.
          </p>
    ...
    return (
      <PublicLayout>
        <div className="flex flex-col items-center justify-center py-20 px-4 text-center">
          ...
          <p className="text-muted-foreground mb-8">
            O link que você acessou expirou ou a barbearia pausou os agendamentos online.
          </p>
          <Button asChild className="h-12 px-8 bg-accent text-accent-foreground font-bold">
            <Link to="/barbearias">Ver barbearias</Link>
          </Button>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-3 mt-3 text-xs font-medium">
                          ...
                          <span className="text-muted-foreground flex items-center gap-1">
                            <Clock className="h-3.5 w-3.5" /> {minutes(s.duration_min)}
                          </span>
                        </div>
                      </button>
                    );
                  ...
                  <div className="flex justify-between gap-4">
                    ...
                    <span className="text-right font-medium">
                      {proId === "any"
                        ...
                        : pros.find((p) => p.id === proId)?.display_name}
                    </span>
                  </div>
                  <div className="flex justify-between gap-4">
          ...
          <div className="hidden md:flex items-center justify-between pt-6 border-t border-border/40">
            ...
            {step < 3 ? (
              <Button
                disabled={!canNext}
                onClick={() => setStep((s) => s + 1)}
                className="h-12 px-8 bg-foreground text-background font-bold"
              >
              ...
              <Button
                disabled={submitting}
                onClick={submit}
                className="h-12 px-8 bg-accent text-accent-foreground font-bold"
              >
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full bg-background/95 backdrop-blur-md border-t border-border/40 p-4 z-40 flex items-center gap-3">
        ...
        {step < 3 ? (
          <Button
            disabled={!canNext}
            onClick={() => setStep((s) => s + 1)}
            className="h-12 flex-1 bg-foreground text-background font-bold uppercase tracking-wider text-xs"
          >
            Continuar <ChevronRight className="ml-2 h-4 w-4" />
          </Button>
        ) : (
          <Button
            disabled={submitting}
            onClick={submit}
            className="h-12 flex-1 bg-accent text-accent-foreground font-bold uppercase tracking-wider text-xs"
          >
