...
import { Search, Users, ShieldOff, ShieldCheck, History, Calendar, Scissors, Phone, Mail, DollarSign, Clock, X, ChevronRight, UserPlus, StickyNote, Ban } from "lucide-react";
    ...
    <div className="flex h-[calc(100vh-4rem)] flex-col bg-background">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-border/40 bg-card/40 p-4 sm:p-5 backdrop-blur-md shrink-0 gap-4">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground">Gerencie a base de clientes da barbearia</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="relative flex-1 sm:w-64">
            ...
            />
            {search && (
              <Button variant="ghost" size="icon" className="absolute right-1 top-1/2 -translate-y-1/2 h-8 w-8 text-muted-foreground hover:text-foreground" onClick={() => setSearch("")}>
                <X className="h-4 w-4" />
              </Button>
            )}
          </div>
          <Button onClick={handleOpenForm} className="bg-accent text-accent-foreground shrink-0 h-11">
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/50 bg-card/20 py-20 text-center">
              ...
              <p className="text-sm text-muted-foreground max-w-sm mt-1">
                {search ? "Tente buscar com outros termos." : "Cadastre o primeiro cliente para começar."}
              </p>
              {!search && (
                <Button onClick={handleOpenForm} className="mt-6 bg-accent text-accent-foreground font-bold">
                  Cadastrar Cliente
                </Button>
              )}
            </div>
              ...
              {customers.map((c) => (
                ...
                >
                  <div className="flex flex-col truncate pr-4">
                    <div className="flex items-center gap-2">
                      <h3 className="font-bold text-foreground text-base truncate">{c.full_name}</h3>
                      {c.blocked && <Badge variant="outline" className="text-[10px] text-destructive border-destructive/30 bg-destructive/10 uppercase">Bloqueado</Badge>}
                    </div>
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground mt-1">
                      {c.phone ? (
                        <span className="flex items-center gap-1"><Phone className="h-3 w-3" /> {c.phone}</span>
                      ) : (
                        <span className="flex items-center gap-1 italic opacity-60"><Phone className="h-3 w-3" /> Sem telefone</span>
                      )}
                      
                      {c.no_show_count > 0 && (
                        <span className="flex items-center gap-1 text-destructive font-medium"><ShieldOff className="h-3 w-3" /> {c.no_show_count} faltas</span>
                      )}
                    </div>
                  </div>
      ...
      <CustomerFormDialog 
        open={formOpen} 
        onClose={() => { setFormOpen(false); if (!selectedCustomer) setSelectedCustomer(null); }} 
        shopId={shopId} 
        customer={selectedCustomer} 
        onSuccess={() => {
          ...
          <div className="max-w-md mx-auto w-full flex flex-col h-full overflow-hidden">
            <DrawerHeader className="text-left border-b border-border/40 shrink-0">
              <div className="flex items-center justify-between">
                <DrawerTitle className="text-xl flex items-center gap-2">
                  {customer.full_name}
                  {customer.blocked && <Badge variant="outline" className="text-[10px] text-destructive border-destructive/30 bg-destructive/10 uppercase">Bloqueado</Badge>}
                </DrawerTitle>
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="p-3 bg-muted/20 border border-border/40 rounded-xl flex flex-col gap-1">
                    <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Total Gasto</span>
                    {statsLoading ? <div className="h-5 bg-muted/50 rounded animate-pulse w-1/2" /> : (
                      <span className="font-mono font-bold text-accent text-lg">{brl(stats?.totalSpent || 0)}</span>
                    )}
                  </div>
                  <div className="p-3 bg-muted/20 border border-border/40 rounded-xl flex flex-col gap-1">
                    <span className="text-xs text-muted-foreground uppercase tracking-wider font-semibold">Visitas</span>
                    {statsLoading ? <div className="h-5 bg-muted/50 rounded animate-pulse w-1/3" /> : (
                      <span className="font-bold text-foreground text-lg">{stats?.totalAppointments || 0}</span>
                    )}
                  </div>
                </div>
                  ...
                  {historyLoading ? (
                    ...
                    <div className="space-y-2">
                      {history.slice(0, 10).map((appt: any) => {
                        ...
                        return (
                          <div key={appt.id} className="flex flex-col p-3 border border-border/40 rounded-lg bg-card text-sm">
                            <div className="flex justify-between items-start mb-1">
                              <span className="font-semibold text-foreground flex items-center gap-1.5">
                                <Calendar className="h-3.5 w-3.5 text-muted-foreground" />
                                {format(new Date(appt.scheduled_start), "dd/MM/yy 'às' HH:mm")}
                              </span>
                              <span className="font-mono text-accent font-bold">{brl(Number(appt.total_amount || 0))}</span>
                            </div>
                      ...
                      {history.length > 10 && (
                        <p className="text-xs text-center text-muted-foreground pt-2">Mostrando os últimos 10 de {history.length}.</p>
                      )}
                    </div>
                  )}
                </div>
            ...
            <DrawerFooter className="border-t border-border/40 pt-3 shrink-0 flex-row gap-2">
              {customer.blocked ? (
                ...
                <Button variant="outline" className="flex-1 text-destructive hover:bg-destructive hover:text-destructive-foreground border-destructive/20" onClick={() => {
                  if (confirm("Bloquear cliente? Ele não poderá fazer agendamentos online.")) blockMut.mutate(customer.id);
                }}>
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md w-full rounded-xl">
        <DialogHeader className="text-left">
          <DialogTitle className="text-xl">{customer ? "Editar Cliente" : "Cadastrar Cliente"}</DialogTitle>
          <DialogDescription>
            {customer ? "Atualize os dados de contato do cliente." : "Adicione um novo cliente à base da barbearia."}
          </DialogDescription>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            <Label htmlFor="c-phone">WhatsApp / Telefone <span className="text-destructive">*</span></Label>
            <Input 
              id="c-phone" 
              type="tel"
          ...
          <div className="space-y-2">
            <Label htmlFor="c-email">E-mail <span className="text-muted-foreground font-normal">(Opcional)</span></Label>
            <Input 
              id="c-email" 
              type="email"
          ...
          <div className="space-y-2">
            <Label htmlFor="c-notes">Anotações Internas <span className="text-muted-foreground font-normal">(Opcional)</span></Label>
            <Textarea 
              id="c-notes" 
