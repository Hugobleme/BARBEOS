      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 p-4 sm:p-5 bg-card/40 backdrop-blur-md shrink-0">
        ...
        <div className="flex items-center gap-2 w-full sm:w-auto">
          {canManage && (
            <>
              <Button
                onClick={() => setPointsFormOpen(true)}
                variant="outline"
                className="flex-1 sm:flex-none h-11 border-accent/30 text-accent hover:bg-accent/10"
              >
            ...
            {loadingRewards ? (
              ...
              <div className="p-6 sm:p-8 text-center border border-dashed border-border/40 rounded-xl bg-muted/10">
                ...
                <p className="text-muted-foreground text-sm mt-1">
                  Comece adicionando a primeira recompensa.
                </p>
              </div>
            ) : (
              <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Card className="border-border/40 overflow-hidden">
              {loadingBalances ? (
                ...
                <div className="p-6 text-center text-sm text-muted-foreground">
                  Nenhum cliente com pontos no momento.
                </div>
              ) : (
                <div className="divide-y divide-border/20">
                  {balances.slice(0, 10).map((b, i) => (
                    <div
                      key={b.id}
                      className="p-3 sm:p-4 flex items-center justify-between hover:bg-card/50 transition-colors"
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="text-sm font-bold">
                            {b.customer?.full_name || "Desconhecido"}
                          </p>
                          <p className="text-[10px] sm:text-xs text-muted-foreground">
                            {b.customer?.phone || "Sem telefone"}
                          </p>
                        </div>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-black text-accent">
                          {b.points}{" "}
                          <span className="text-[10px] text-muted-foreground uppercase font-normal tracking-wider">
                            pts
                          </span>
                        </p>
                      </div>
...
function RewardForm({ open, onClose, shopId, reward, onSuccess }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(pts) || pts <= 0)
      return toast.error("Os pontos necessários devem ser maiores que zero.");
        ...
        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              placeholder="Ex: Válido de ter a sex"
              className="h-11"
            />
          </div>
...
function PointsForm({ open, onClose, shopId, rewards, onSuccess }: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(pts) || pts <= 0)
      return toast.error("A quantidade de pontos deve ser maior que zero.");
      ...
      <DialogContent className="max-w-md w-[95vw] rounded-xl max-h-[90vh] overflow-y-auto">
        ...
        {!customer ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4 py-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {rewards.map((r: any) => (
                    <Badge
                      ...
                      onClick={() => {
                        setPoints(r.sessions_total.toString());
                        setReason(`Resgate: ${r.name}`);
                      }}
                    >
            ...
            <div className="space-y-2">
              <Label>
                Quantidade de Pontos <span className="text-destructive">*</span>
              </Label>
              <Input
                type="number"
                min="1"
