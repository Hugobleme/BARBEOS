...
function AdminHorariosPage() {
        ...
        for (const inv of intervals) {
          if (inv.opens_at && inv.closes_at) {
            toInsert.push({
              ...
              opens_at: inv.opens_at,
              closes_at: inv.closes_at,
            });
          }
        }
      }
  ...
  const addInterval = (day: number) => {
    setEditing((prev) => ({
      ...prev,
      [day]: [...prev[day], { opens_at: "09:00", closes_at: "18:00" }],
    }));
  };
        ...
        <header className="px-6 py-4 border-b border-border/40 flex items-center justify-between shrink-0">
          <div>
            ...
            <p className="text-xs text-muted-foreground uppercase tracking-wider">
              Configure quando sua barbearia está aberta
            </p>
          </div>
          {!isEditing && (
            <Button
              onClick={handleEdit}
              className="bg-accent text-accent-foreground font-bold text-xs uppercase tracking-wider"
            >
              Editar Horários
            </Button>
          )}
                  ...
                  <div className="space-y-4">
                    {WEEKDAYS.map((name, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex flex-col sm:flex-row sm:items-center py-3 border-b border-border/40 last:border-0 gap-4"
                        >
                  ...
                  <Card key={i} className="p-4 flex flex-col gap-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={() => addInterval(i)}>
                        ...
                        {editing[i].map((inv, idx) => (
                          <div key={idx} className="flex items-center gap-2">
                            <Input
                              type="time"
                              value={inv.opens_at}
                              onChange={(e) => updateInterval(i, idx, "opens_at", e.target.value)}
                              className="w-32"
                            />
                            ...
                            <Input
                              type="time"
                              value={inv.closes_at}
                              onChange={(e) => updateInterval(i, idx, "closes_at", e.target.value)}
                              className="w-32"
                            />
                            <Button
                              variant="ghost"
                              size="icon"
                              onClick={() => remInterval(i, idx)}
