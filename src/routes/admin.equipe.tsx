...
function EquipePage() {
  ...
  const handleRemove = (m: any) => {
    ...
    if (
      confirm(
        `Tem certeza que deseja remover o acesso de ${m.profile?.full_name || "este membro"}?`,
      )
    ) {
      ...
      <div className="flex items-center justify-between border-b border-border/40 bg-card/40 p-4 sm:p-5 backdrop-blur-md shrink-0">
        ...
        {canManage && (
          <Button
            ...
            className="bg-accent text-accent-foreground shrink-0 h-11 px-4"
          >
        ...
        <div className="mx-auto max-w-4xl p-4 sm:p-6 pb-24 space-y-6">
          <div className="space-y-3">
            ...
            {isLoading ? (
              ...
              <div className="space-y-3">
                {members.map((m) => {
                  const roleCfg = ROLE_LABELS[m.role as string] || {
                    label: m.role,
                    color: "border-border",
                  };
                  const canRemove =
                  ...
                  return (
                    <Card
                      key={m.id}
                      className="group flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-border/40 transition-colors hover:border-accent/50 bg-card"
                    >
                      <div className="flex items-center gap-4">
                        ...
                        <div className="space-y-0.5">
                          <h3 className="font-bold text-foreground text-base leading-none">
                            {m.profile?.full_name || "Usuário sem nome"}{" "}
                            {m.profile_id === user?.id && (
                          ...
                          <Badge
                            variant="outline"
                            className={`text-[10px] uppercase font-bold tracking-wider mt-1 ${roleCfg.color}`}
                          >
                            {roleCfg.label}
                          </Badge>
                        </div>
            ...
            <div className="space-y-3 pt-6 border-t border-border/40">
              ...
              <div className="space-y-3">
                {invitations.map((inv) => (
                  <Card
                    key={inv.id}
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-4 rounded-xl border border-border/40 bg-card/40 border-dashed"
                  >
                    <div className="space-y-1">
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          variant="outline"
                          className="text-[10px] uppercase bg-muted text-muted-foreground border-border"
                        >
                          Pendente
                        </Badge>
                      </div>
...
function InviteDialog({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const { error } = await supabase.from("barbershop_invitations").insert({
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md w-full rounded-xl">
        <DialogHeader>
          <DialogTitle>
            {generatedToken ? "Convite Gerado!" : "Convidar Membro da Equipe"}
          </DialogTitle>
          <DialogDescription>
            {generatedToken
          ...
          <div className="space-y-4 py-4">
            <div className="p-4 bg-emerald-500/10 border border-emerald-500/20 rounded-xl flex items-start gap-3 text-emerald-600">
              ...
              <p className="text-sm font-medium">
                Link válido por 7 dias. O usuário precisará criar uma conta com o e-mail convidado.
              </p>
            </div>
            <div className="flex gap-2">
              <Input
                readOnly
                value={`${window.location.origin}/invite?token=${generatedToken}`}
