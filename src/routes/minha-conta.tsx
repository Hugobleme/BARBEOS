...
function MinhaContaPage() {
  ...
  const upcoming = appointments.filter(
    (a) =>
      isFuture(new Date(a.scheduled_start)) && !["cancelled", "no_show"].includes(a.status || ""),
  );
  const history = appointments
    .filter(
      (a) =>
        isPast(new Date(a.scheduled_start)) || ["cancelled", "no_show"].includes(a.status || ""),
    )
    .reverse();
        ...
        <div className="mx-auto max-w-4xl px-4 flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm md:text-base">
              Bem-vindo à sua área exclusiva BarberOS.
            </p>
          </div>
      ...
      <div className="mx-auto max-w-4xl px-4 py-8 pb-32">
              ...
              <div className="relative z-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                  <h3 className="font-serif text-xl md:text-2xl font-bold mb-1">
                    {nextAppt.barbershop?.name}
                  </h3>
                  <div className="flex flex-wrap items-center gap-3 text-sm md:text-base font-medium opacity-90">
                    <span className="flex items-center gap-1.5">
                      <Calendar className="h-4 w-4" />{" "}
                      {format(new Date(nextAppt.scheduled_start), "EEEE, dd/MM", { locale: ptBR })}
                    </span>
                    <span className="flex items-center gap-1.5">
                      <Clock className="h-4 w-4" />{" "}
                      {format(new Date(nextAppt.scheduled_start), "HH:mm")}
                    </span>
                  </div>
                  <p className="mt-3 text-sm opacity-80 line-clamp-1">
                    {nextAppt.services?.map((s: any) => s.service?.name).join(" + ")} com{" "}
                    {nextAppt.professional?.display_name || "Profissional"}
                  </p>
                </div>
                <div className="flex items-center gap-2 mt-2 md:mt-0">
                  <Button
                    asChild
                    variant="secondary"
                    className="h-11 bg-background text-foreground hover:bg-background/90 w-full md:w-auto font-bold"
                  >
                    <Link to="/b/$slug" params={{ slug: nextAppt.barbershop?.slug || "" }}>
                      Ver barbearia
                    </Link>
                  </Button>
                </div>
        ...
        <div className="min-h-[400px]">
          {isLoading ? (
            ...
            upcoming.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-border/60 rounded-xl bg-card/20">
                ...
                <p className="text-muted-foreground mt-1 mb-6">
                  Que tal marcar um horário para dar um tapa no visual?
                </p>
            ...
            history.length === 0 ? (
              <div className="flex flex-col items-center justify-center p-12 text-center border border-border/40 rounded-xl bg-card/20">
                ...
                >
                  <Link to="/barbearias">Explorar barbearias</Link>
                </Button>
              </div>
            ) : (
              <div className="grid gap-4">
            ...
            <Card className="p-5 md:p-8 max-w-xl border-border/60">
              ...
              <p className="text-muted-foreground text-sm mb-6">
                Cadastre sua barbearia para começar a gerenciar agendamentos, profissionais e
                clientes pelo sistema.
              </p>

              <form
                onSubmit={handleCreateBarbershop}
                className="space-y-5 bg-card/50 p-6 rounded-xl border border-border/50"
              >
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    value={bsPhone}
                    onChange={(e) => setBsPhone(phoneMask(e.target.value))}
                    className="h-11 bg-background"
                ...
                <Button
                  type="submit"
                  disabled={creatingBs}
                  className="w-full h-12 bg-accent text-accent-foreground font-bold text-sm tracking-wider uppercase mt-2"
                >
            ...
            <Card className="p-5 md:p-8 max-w-xl border-border/60">
              ...
              <form onSubmit={saveProfile} className="space-y-5">
                ...
                <Button
                  ...
                  className="w-full sm:w-auto h-11 bg-foreground text-background font-bold mt-4"
                >
...
function AppointmentCard({ appt, cancelable, onCancel }: any) {
  ...
  const canReview =
    isPastAppt &&
    !isCanceled &&
    (!appt.satisfaction_surveys || appt.satisfaction_surveys.length === 0);

  return (
    <Card
      className={`p-4 md:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 border-border/60 transition-colors ${isCanceled ? "opacity-70 bg-muted/20" : "bg-card hover:border-accent/40"}`}
    >
      <div className="flex-1">
        ...
        <p className="text-sm text-muted-foreground line-clamp-2">
          {appt.services?.map((s: any) => s.service?.name).join(" + ")} com{" "}
      ...
      <div className="flex flex-col gap-2 min-w-[140px] pt-4 md:pt-0 border-t md:border-0 border-border/40">
        {!isCanceled && appt.barbershop?.slug && (
          <Button asChild variant="outline" className="h-10 text-xs w-full">
            <Link to="/b/$slug" params={{ slug: appt.barbershop.slug }}>
              Ver barbearia
            </Link>
          </Button>
        )}
        {cancelable && !isCanceled && !isPastAppt && (
          <Button
            variant="ghost"
            className="h-10 text-xs text-destructive hover:bg-destructive/10 w-full"
            onClick={onCancel}
          >
            Cancelar
          </Button>
        ...
        {canReview && (
          <Button
            asChild
            className="h-10 text-xs bg-accent text-accent-foreground font-bold w-full"
          >
        ...
        {!canReview && isPastAppt && !isCanceled && appt.satisfaction_surveys?.length > 0 && (
          <Badge
            variant="secondary"
            className="justify-center h-10 rounded-md bg-muted/50 text-muted-foreground"
          >
            Avaliado ✓
          </Badge>
        )}
