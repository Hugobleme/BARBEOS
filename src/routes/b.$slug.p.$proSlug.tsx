      ...
      <PublicLayout>
        <div className="mx-auto max-w-2xl px-4 py-16 text-center">
          ...
          <p className="text-muted-foreground mb-8">
            Este profissional não está disponível ou não existe.
          </p>
          ...
          <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
            <div className="flex items-center gap-5">
              <Avatar className="h-24 w-24 md:h-32 md:w-32 border-4 border-background shadow-lg">
                ...
                <AvatarFallback className="text-3xl font-serif bg-muted text-muted-foreground">
              ...
              <div>
                ...
                <p className="text-accent font-medium text-lg mt-1">
                  {pro.specialties || "Barbeiro Especialista"}
                </p>
                {shop_rating && (
                  <Badge
                    variant="secondary"
                    className="mt-3 flex w-fit items-center gap-1 font-bold"
                  >
            ...
            <div className="hidden md:block">
              <Button
                asChild
                className="h-12 px-8 font-bold text-base bg-accent text-accent-foreground shadow-lg hover:scale-105 transition-transform"
              >
          ...
          <section>
            ...
            <div className="grid gap-3 sm:grid-cols-2 md:grid-cols-3">
              {services.map((s) => (
                <Card
                  key={s.id}
                  className="p-4 border-border/40 hover:border-accent/40 transition-colors"
                >
          ...
          <section>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {portfolio.map((img) => (
                <div
                  key={img.id}
                  className="aspect-square rounded-xl overflow-hidden bg-muted border border-border/40 shadow-sm"
                >
                  <img
                    src={img.image_url}
                    alt="Trabalho"
                  ...
                  <div className="flex items-center gap-2 mb-2">
                    <div className="flex">
                      {[1, 2, 3, 4, 5].map((star) => (
                        <Star
                          key={star}
                          className={`h-3.5 w-3.5 ${star <= r.shop_rating ? "fill-accent text-accent" : "text-muted-foreground/30"}`}
                        />
                      ))}
                    </div>
