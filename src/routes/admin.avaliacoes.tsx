            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-1 mt-1">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    key={star}
                    className={`h-4 w-4 ${star <= Math.round(stats.avg) ? "text-accent fill-accent" : "text-muted-foreground/30"}`}
                  />
                ))}
              </div>
            ...
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border/50 bg-card/20 py-20 text-center">
              ...
              <h3 className="font-serif text-lg font-bold text-foreground">
                {filterRating !== "all"
                  ...
                  : "Sua barbearia ainda não tem avaliações."}
              </h3>
            </div>
          ) : (
            <div className="space-y-4">
              {filtered.map((s) => {
                ...
                return (
                  <Card
                    key={s.id}
                    className={`flex flex-col p-4 sm:p-5 rounded-xl border border-border/40 transition-colors bg-card ${!s.is_public ? "opacity-70" : ""}`}
                  >
                      ...
                      <div className="flex flex-col gap-1">
                        <div className="flex items-center gap-2">
                          <div className="flex items-center gap-0.5">
                            {[1, 2, 3, 4, 5].map((star) => (
                              <Star
                                key={star}
                                className={`h-3.5 w-3.5 ${star <= rating ? "text-accent fill-accent" : "text-muted-foreground/30"}`}
                              />
                            ))}
                          </div>
                          ...
                          {!s.is_public && (
                            <Badge
                              variant="outline"
                              className="text-[10px] uppercase bg-muted/50 text-muted-foreground ml-2"
                            >
                        ...
                        <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground mt-1">
                          ...
                          <span className="flex items-center gap-1">
                            ...
                            {isNaN(new Date(s.answered_at).getTime())
                              ? "Inv�lido"
                              : format(new Date(s.answered_at), "dd/MM/yy")}
                          </span>
                          {s.professional && (
                            <span className="flex items-center gap-1 bg-muted/50 px-2 py-0.5 rounded-md">
                              Atendido por: {s.professional.display_name}
                            </span>
                          )}
                        </div>
                      </div>
                    ...
                    <div className="flex items-center justify-end mt-4 pt-3 border-t border-border/40">
                      ...
                      {s.is_public && (
                        <Button
                          variant="ghost"
                          size="sm"
                          className="h-8 text-xs text-destructive hover:bg-destructive/10 ml-2"
                          onClick={() => {
                            if (confirm("Deseja ocultar esta avaliação do seu perfil público?"))
                              hideMut.mutate(s.id);
                          }}
                        >
                          Ocultar
