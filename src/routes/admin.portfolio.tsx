  ...
  return (
    <div className="flex h-[calc(100vh-4rem)] flex-col bg-background">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 p-4 sm:p-5 bg-card/40 backdrop-blur-md shrink-0">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          <Button
            ...
            className="bg-accent text-accent-foreground shrink-0 h-11 w-full sm:w-auto"
          >
          ...
          {isLoading ? (
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {[1, 2, 3, 4, 5, 6].map((i) => (
                <Card
                  key={i}
                  className="aspect-square animate-pulse rounded-xl border border-border/40 bg-muted/30"
                />
              ))}
            </div>
          ) : isError ? (
            <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-destructive/30 rounded-xl bg-destructive/5 text-destructive mt-8">
              <p className="font-bold mb-2">
                O portfólio ainda não está configurado nesta unidade.
              </p>
              <p className="text-sm opacity-80 max-w-md mx-auto mb-4">
                Estamos preparando a infraestrutura para que você possa exibir seus trabalhos em
                breve.
              </p>
              <Button
                onClick={() => refetch()}
            ...
            <div className="flex flex-col items-center justify-center p-12 text-center border border-dashed border-border/40 rounded-xl bg-muted/10 mt-8">
              ...
              <p className="text-sm text-muted-foreground max-w-md mt-1">
                Comece adicionando o primeiro trabalho.
              </p>
              <Button
                onClick={() => setFormOpen(true)}
                variant="outline"
                className="mt-6 border-accent/30 text-accent hover:bg-accent/10"
              >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4">
              {items.map((it) => {
                const pro = it.professional_id
                  ? professionals.find((p) => p.id === it.professional_id)
                  : null;

                return (
                  <Card
                    key={it.id}
                    className="group relative rounded-xl border border-border/40 overflow-hidden bg-card flex flex-col h-full shadow-sm hover:shadow-md transition-all"
                  >
                      ...
                      <img
                        ...
                        onError={(e) => {
                          (e.target as HTMLImageElement).src =
                            'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="100" height="100" viewBox="0 0 24 24" fill="none" stroke="%2394a3b8" stroke-width="1" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>';
      ...
      <PortfolioForm
        ...
        onSuccess={() => qc.invalidateQueries({ queryKey: ["admin-portfolio", shopId] })}
      />
    </div>
  );
}
    ...
    <Dialog open={open} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-w-md w-full rounded-xl">
        <DialogHeader className="text-left">
          ...
          <DialogDescription>Adicione uma imagem externa ao portfólio.</DialogDescription>
        </DialogHeader>

        <form onSubmit={handleSubmit} className="space-y-4 py-2">
          <div className="space-y-2">
            <Label>
              URL da Imagem <span className="text-destructive">*</span>
            </Label>
            <Input
              type="url"
            ...
            {formData.image_url && (
              <div className="mt-2 aspect-[4/3] sm:aspect-video w-full rounded-lg bg-muted border border-border/40 overflow-hidden flex items-center justify-center">
                <img
                  ...
                  onError={(e) => {
                    (e.target as HTMLImageElement).src =
                      'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="%2394a3b8" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"></rect><circle cx="8.5" cy="8.5" r="1.5"></circle><polyline points="21 15 16 10 5 21"></polyline></svg>';
