...
function RelatoriosPage() {
  ...
  const dateRange = useMemo(() => {
    ...
    switch (period) {
      case "hoje":
        return { s: startOfDay(today), e: endOfDay(today) };
      case "7d":
        return { s: startOfDay(subDays(today, 6)), e: endOfDay(today) };
      case "mes":
        return { s: startOfMonth(today), e: endOfDay(today) };
      case "mes_passado": {
      ...
      case "custom":
        return {
          s: startOfDay(new Date(customStart + "T12:00:00")),
          e: endOfDay(new Date(customEnd + "T12:00:00")),
        };
      default:
        return { s: startOfMonth(today), e: endOfDay(today) };
    }
  ...
  const avgTicket = revData?.transactionsCount
    ? revData.totalRevenue / revData.transactionsCount
    : 0;
  ...
  const exportCSV = () => {
    ...
    if (topCust && topCust.length > 0) {
      ...
      topCust.forEach(
        (c) =>
          (csv += `${c.customer?.full_name || "Desconhecido"};${c.appointmentsCount};${c.totalSpent}\n`),
      );
    }
    ...
    link.setAttribute(
      "download",
      `relatorio-barbeos-${format(dateRange.s, "yyyy-MM-dd")}-a-${format(dateRange.e, "yyyy-MM-dd")}.csv`,
    );
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 p-4 sm:p-5">
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
              Métricas e performance da barbearia
            </p>
          </div>
          ...
          <Button
            ...
            variant="outline"
            className="border-accent/30 text-accent hover:bg-accent/10 h-11"
          >
          ...
          {period === "custom" && (
            <div className="flex items-center gap-2">
              <Input
                ...
                className="w-[140px] h-10 bg-background"
              />
          ...
          {isLoading ? (
            ...
            <>
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 sm:gap-4">
                <KPICard
                  title="Faturamento"
                  value={brl(revData.totalRevenue)}
                  icon={DollarSign}
                  color="text-emerald-500"
                />
                <KPICard
                  title="Operações (Vendas)"
                  value={revData.transactionsCount}
                  icon={TrendingUp}
                  color="text-accent"
                />
                <KPICard
                        ...
                        >
                          <div className="flex flex-col truncate pr-2">
                            ...
                            <span className="text-xs text-muted-foreground">
                              {svc.bookingsCount} vezes
                            </span>
                          </div>
                          <span className="font-mono text-emerald-500">
                            {brl(svc.totalRevenue)}
                          </span>
                        </div>
                      ))
                    )}
                ...
                <Card className="p-4 bg-card border-border/40 flex flex-col h-full rounded-xl md:col-span-2 lg:col-span-1">
                  ...
                  <div className="flex-1 space-y-3">
                    {!topCust || topCust.length === 0 ? (
                      ...
                      topCust.map((c: any) => (
                        <div
                          key={c.customer?.id || Math.random()}
                          className="flex justify-between items-center text-sm border-b border-border/40 pb-2 last:border-0 last:pb-0"
                        >
                          <div className="flex flex-col truncate pr-2">
                            <span className="font-bold truncate">
                              {c.customer?.full_name || "Desconhecido"}
                            </span>
                            <span className="text-xs text-muted-foreground">
                              {c.appointmentsCount} visitas
                            </span>
                          </div>
...
function EmptyState({ text }: { text: string }) {
  return (
    <div className="h-full flex items-center justify-center text-sm text-muted-foreground italic bg-muted/20 rounded-lg p-4 text-center border border-dashed border-border/40">
      {text}
    </div>
  );
}
