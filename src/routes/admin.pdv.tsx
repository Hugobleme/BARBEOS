...
function PDV() {
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(
      (p) =>
        p.name.toLowerCase().includes(search.toLowerCase()) && p.active && (p.stock_qty || 0) > 0,
    );
  }, [products, search]);
  ...
  return (
    <div className="flex flex-col lg:flex-row h-[calc(100vh-4rem)] bg-background overflow-hidden">
        ...
        <div className="flex flex-col gap-3 p-4 border-b border-border/40 bg-card/40 shrink-0">
          <div>
            ...
            <p className="text-xs sm:text-sm text-muted-foreground">
              Registre serviços e produtos diretamente no caixa
            </p>
          </div>
      ...
      {cart.length > 0 && !showMobileCart && (
        <div
          className="lg:hidden p-4 border-t border-border/40 bg-accent text-accent-foreground flex items-center justify-between shrink-0"
...
function ItemCard({ item, type, onAdd }: any) {
  ...
  return (
    <Card
      className="flex items-center justify-between p-3 rounded-xl border border-border/40 bg-card hover:border-accent/40 transition-colors cursor-pointer"
      onClick={onAdd}
    >
      ...
      <Button
        size="icon"
        variant="ghost"
        className="shrink-0 h-8 w-8 rounded-full text-accent hover:bg-accent/10"
      >
    ...
    <div className="flex flex-col h-full bg-card/50">
      ...
      <div className="p-4 border-b border-border/40 shrink-0 space-y-2">
        ...
        <Select value={customerId} onValueChange={setCustomerId}>
          ...
          <SelectContent>
            <SelectItem value="avulso" className="font-semibold text-accent">
              Nenhum (Venda Avulsa)
            </SelectItem>
            {customers.map((c: any) => (
          ...
          <div className="p-4 space-y-3">
            {cart.map((item: CartItem) => (
              <div
                ...
                className="flex flex-col gap-2 p-3 border border-border/40 rounded-xl bg-background"
              >
                <div className="flex justify-between items-start gap-2">
                  ...
                  <Button
                    variant="ghost"
                    size="icon"
                    className="h-6 w-6 text-destructive shrink-0 -mr-1"
