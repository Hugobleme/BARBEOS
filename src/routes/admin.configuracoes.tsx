...
function ConfigPage() {
  ...
  const publicUrl =
    typeof window !== "undefined"
      ? `${window.location.origin}/b/${barbershop?.slug || formData.slug}`
      : `/b/${barbershop?.slug || formData.slug}`;
        ...
        <div className="max-w-3xl mx-auto space-y-8 pb-12">
          <header className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Mantenha as informações da sua unidade atualizadas para uma presença pública mais
              clara.
            </p>
          </header>
          ...
          <section className="space-y-4">
            ...
            <Card className="p-4 sm:p-5 bg-card border-border/60 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-start sm:items-center gap-3">
                <div
                  className={`mt-0.5 sm:mt-0 p-2 rounded-full shrink-0 ${hasSlug ? "bg-emerald-500/10 text-emerald-500" : "bg-amber-500/10 text-amber-500"}`}
                >
                  {hasSlug ? (
                ...
                <div>
                  <p className="font-bold text-sm">
                    {hasSlug
                      ...
                      : "Seu perfil público ainda precisa de um endereço."}
                  </p>
                  {hasSlug && (
                    <a
                      href={publicUrl}
                      target="_blank"
                      rel="noreferrer"
                      className="text-xs text-muted-foreground hover:text-accent truncate max-w-[200px] sm:max-w-xs inline-block"
                    >
                      {publicUrl}
                    </a>
                  )}
              ...
              <div className="shrink-0">
                {hasSlug ? (
                  <Button
                    ...
                    variant="outline"
                    size="sm"
                    className="w-full sm:w-auto text-xs font-bold uppercase tracking-wider border-accent/40 text-accent hover:bg-accent/10"
                  >
          ...
          {isLoading ? (
            ...
            <form onSubmit={handleSave} className="space-y-8">
                ...
                <Card className="p-4 sm:p-6 bg-card border-border/40 shadow-sm space-y-6">
                  ...
                  <div className="space-y-2">
                    <Label htmlFor="slug-input">
                      Endereço do perfil (Slug) <span className="text-destructive">*</span>
                    </Label>
                    ...
                    <p className="text-[11px] text-muted-foreground">
                      Apenas letras, números e hifens. Ex: barbearia-do-ze
                    </p>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-col sm:flex-row gap-3 sm:items-center">
                      ...
                      <div className="flex-1 space-y-1">
                        ...
                        />
                        <p className="text-[11px] text-muted-foreground">
                          Insira o link direto de uma imagem pública para usar como logo.
                        </p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <Label className="flex items-center gap-2">
                          <Phone className="h-3 w-3" /> Telefone Fixo
                        </Label>
                        <Input
                      ...
                      <div className="space-y-2">
                        <Label className="flex items-center gap-2">
                          <Globe className="h-3 w-3 text-blue-500" /> Website
                        </Label>
                        <Input
              ...
              <div>
                <Button
                  type="submit"
                  className="w-full sm:w-auto h-11 bg-accent text-accent-foreground font-bold px-8 text-xs uppercase tracking-wider"
            ...
            <section className="space-y-4 pt-8">
              ...
              <Card className="p-4 sm:p-6 bg-muted/10 border-border/40 shadow-sm space-y-4">
                <p className="text-sm text-muted-foreground mb-4">
                  Seu perfil público exibe as seguintes informações quando configuradas:
                </p>
                <div className="space-y-3">
                  <div className="flex items-center gap-3 text-sm">
                    <CheckCircle2
                      className={`h-4 w-4 ${barbershop.name ? "text-emerald-500" : "text-muted-foreground/30"}`}
                    />
                    <span className={barbershop.name ? "text-foreground" : "text-muted-foreground"}>
                      Nome da barbearia
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-sm">
                    <CheckCircle2
                      className={`h-4 w-4 ${formData.address || formData.city ? "text-emerald-500" : "text-muted-foreground/30"}`}
                    />
                    <span
                      className={
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    <CheckCircle2
                      className={`h-4 w-4 ${onboarding?.isServicesComplete ? "text-emerald-500" : "text-muted-foreground/30"}`}
                    />
                    <span
                      className={
                        onboarding?.isServicesComplete ? "text-foreground" : "text-muted-foreground"
                      }
                    >
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    <CheckCircle2
                      className={`h-4 w-4 ${onboarding?.isProsComplete ? "text-emerald-500" : "text-muted-foreground/30"}`}
                    />
                    <span
                      className={
                        onboarding?.isProsComplete ? "text-foreground" : "text-muted-foreground"
                      }
                    >
                  ...
                  <div className="flex items-center gap-3 text-sm">
                    <CheckCircle2
                      className={`h-4 w-4 ${barbershop.description ? "text-emerald-500" : "text-muted-foreground/30"}`}
                    />
                    <span
                      className={
                        barbershop.description ? "text-foreground" : "text-muted-foreground"
                      }
                    >
                      Descrição
                    </span>
                ...
                <div className="pt-4 mt-4 border-t border-border/40">
                  <Button
                    asChild
                    variant="link"
                    className="px-0 text-xs font-bold uppercase tracking-wider text-accent h-auto"
                  >
                    <Link to="/admin/onboarding">
                      Ver primeiros passos <ArrowRight className="ml-1 h-3 w-3" />
                    </Link>
                  </Button>
                </div>
              </Card>
            ...
            <section className="space-y-4 pt-12 border-t border-border/20">
              ...
              <Card className="p-4 sm:p-6 bg-destructive/5 border-destructive/20 rounded-xl shadow-sm">
                ...
                <p className="text-sm text-muted-foreground mt-1 mb-4">
                  Ao desativar, sua barbearia deixará de aparecer para clientes e bloqueará novos
                  agendamentos. Você poderá reativá-la solicitando ao suporte. Nenhum dado será
                  excluído permanentemente nesta ação.
                </p>
                <Button
                  variant="outline"
                  className="w-full sm:w-auto text-xs font-bold uppercase tracking-wider border-destructive/30 text-destructive hover:bg-destructive/10"
                  onClick={() => {
                    const check = prompt(
                      `Digite "${barbershop.name}" para confirmar a desativação:`,
                    );
