...
function AdminComissoesPage() {
  ...
  const handlePay = (c: Commission) => {
    if (
      confirm(`Confirmar o pagamento de ${brl(c.amount)} para ${c.professional?.display_name}?`)
    ) {
      payMut.mutate(c.id);
    }
  };
  ...
  const summary = useMemo(() => {
    ...
    filtered.forEach((c) => {
      ...
      if (!byPro[c.professional_id])
        byPro[c.professional_id] = { name: c.professional.display_name, total: 0 };
    ...
    return {
      totalPending,
      totalPaid,
      byPro: Object.values(byPro).sort((a, b) => b.total - a.total),
    };
  }, [filtered]);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border/40 p-4 sm:p-5 bg-card/40 backdrop-blur-md shrink-0">
        <div>
          ...
          <p className="text-xs sm:text-sm text-muted-foreground mt-0.5">
            Acompanhamento e pagamento de profissionais
          </p>
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5 border-b border-border/40 bg-card/20 shrink-0">
        <div className="flex flex-col sm:flex-row gap-3 flex-wrap">
          ...
          <Select value={year} onValueChange={setYear}>
            <SelectTrigger className="w-full sm:w-[100px] h-11">
                        ...
                        <tr key={c.id} className="hover:bg-muted/20 transition-colors group">
                          ...
                          <td className="px-4 sm:px-6 py-3 text-center">
                            {c.status === "paid" ? (
                              <Badge
                                variant="outline"
                                className="border-emerald-500/30 bg-emerald-500/10 text-emerald-500 text-[10px] uppercase font-bold"
                              >
                                Paga
                              </Badge>
                            ) : (
                              <Badge
                                variant="outline"
                                className="border-orange-500/30 bg-orange-500/10 text-orange-500 text-[10px] uppercase font-bold"
                              >
                                Pendente
                              </Badge>
                            )}
