...
function BarbershopProfilePage() {
  ...
  const locationText = [addr.street, addr.neighborhood, addr.city, addr.state]
    .filter(Boolean)
    .join(", ");
  ...
  return (
    <PublicLayout>
      ...
      <div className="relative h-64 md:h-80 w-full bg-muted overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 w-full p-4 md:p-8 max-w-5xl mx-auto flex flex-col md:flex-row md:items-end justify-between gap-4">
          ...
          <div className="hidden md:block mb-4">
            <Button
              asChild
              className="h-12 px-8 font-bold text-base bg-accent text-accent-foreground hover:bg-accent/90 shadow-lg transition-transform hover:scale-105"
            >
            ...
            {services.length === 0 ? (
              ...
              <div className="grid gap-3">
                {services.map((s: any) => (
                  <Card
                    key={s.id}
                    className="p-4 flex items-center justify-between gap-4 border-border/40 hover:border-accent/40 transition-colors"
                  >
                    <div>
                      ...
                      <div className="flex items-center gap-3 mt-2 text-xs font-medium">
                        ...
                        <span className="text-muted-foreground flex items-center gap-1">
                          <Clock className="h-3.5 w-3.5" /> {minutes(s.duration)}
                        </span>
                      </div>
                    </div>
                    <Button
                      asChild
                      variant="outline"
                      className="h-10 shrink-0 border-accent/30 text-accent hover:bg-accent hover:text-accent-foreground"
                    >
            ...
            <section>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {professionals.map((p: any) => (
                  <Card
                    key={p.id}
                    className="p-4 flex flex-col justify-between border-border/40 hover:border-accent/40 transition-colors group"
                  >
                    <div className="flex items-center gap-3 mb-4">
                      <Avatar className="h-12 w-12 border-2 border-background shadow-sm">
                        <AvatarImage src={p.avatar_url || ""} />
                        <AvatarFallback className="bg-muted text-muted-foreground font-serif">
                    ...
                    <div className="grid grid-cols-2 gap-2 mt-auto">
                      {p.slug ? (
                        <Button asChild variant="ghost" size="sm" className="h-9 text-xs">
                          <Link to="/b/$slug/p/$proSlug" params={{ slug, proSlug: p.slug }}>
                            Ver perfil
                          </Link>
                        </Button>
                      ) : (
                      ...
                      <Button
                        ...
                        variant="secondary"
                        size="sm"
                        className="h-9 text-xs bg-accent/10 text-accent hover:bg-accent hover:text-accent-foreground"
                      >
                        <Link to="/agendar" search={{ barbershop: slug, professional: p.id }}>
                          Agendar
                        </Link>
                      </Button>
                    </div>
                    ...
                    <div className="flex items-center gap-2 mb-2">
                      <div className="flex">
                        {[1, 2, 3, 4, 5].map((star) => (
                          <Star
                            key={star}
                            className={`h-3.5 w-3.5 ${star <= r.rating ? "fill-accent text-accent" : "text-muted-foreground/30"}`}
                          />
                        ))}
                      </div>
      ...
      <div className="md:hidden fixed bottom-0 left-0 w-full p-4 bg-background/80 backdrop-blur-md border-t border-border/40 z-40">
        <Button
          asChild
          className="w-full h-12 font-bold text-base bg-accent text-accent-foreground shadow-lg"
        >
          <Link to="/agendar" search={{ barbershop: slug }}>
            Agendar Horário <Calendar className="ml-2 h-5 w-5" />
          </Link>
