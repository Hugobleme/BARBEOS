...
const STATUS = {
  ...
  cancelled: {
    label: "Cancelado",
    className: "bg-muted/50 text-muted-foreground line-through border-transparent",
  },
  no_show: {
    label: "Faltou",
  ...
  return (
    ...
    >
      <Card className="group relative overflow-hidden border-none bg-card/50 shadow-xl shadow-black/5 backdrop-blur-md transition-all hover:bg-card/80">
        <div
          className={`absolute left-0 top-0 h-full w-1.5 ${st.className.split(" ")[1].replace("text-", "bg-")}`}
        />
          ...
          <div className="flex flex-1 items-start gap-3 sm:gap-5">
            ...
            <div className="min-w-0 space-y-1">
              <div className="flex flex-wrap items-center gap-x-2 gap-y-1">
                ...
                <Badge
                  variant="outline"
                  className={`h-5 px-2 py-0 text-[9px] font-black uppercase tracking-tight border-transparent ${st.className}`}
                >
                  {st.label}
                </Badge>
              </div>
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-9 w-9 rounded-xl text-muted-foreground/40 hover:bg-muted/50 hover:text-foreground"
                >
              ...
              <DropdownMenuContent
                align="end"
                className="w-48 rounded-2xl border-border/40 bg-background/95 backdrop-blur-xl"
              >
